...
# BELOW: DELETE WHEN FREEZE 1 IS READY
...
#     }
#     graphs_paths = {
#         'hifiasm': 'output/target_assembly/{chrom}/hifiasm/{sample}/{sample}_{read_type}.{mapq}.{tigs}.gfa',
#         'mbg': 'output/target_assembly/{chrom}/mbg/{sample}/{sample}_{read_type}.{mapq}.k{kmer}-w{window}-r{resolvek}.gfa',
#         'lja': 'no_uncompressed_graph_available'
#     }
#     try:
#         # for hifiasm whole-genome graphs
#         gfa = SAMPLE_DATA[wildcards.sample][wildcards.tigs]
#     except KeyError:
...
#         template = 'output/read_subsets/{chrom}/{sample}_ONTUL.{chrom}-reads.{mapq}.fasta.gz'
#         formatter = {
#             'sample': wildcards.sample,
#             'sample_info': wildcards.sample_info,
#             'mapq': mapq,
...
#     else:
#         ont_reads = str(SAMPLE_DATA[wildcards.sample][wildcards.ont_type])
